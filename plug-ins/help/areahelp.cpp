...
void AreaHelp::getRawText( Character *ch, ostringstream &in ) const
{
    ...
    /* The "Карта:" row (the [map] client button + its maps.html link) was
     * dropped from area help on request (Bugs 8700): the web client carries its
     * own live map pane, so the static link/button was redundant clutter. The
     * client's [map=...] regex now has no server emitter and is dead-but-inert. */
