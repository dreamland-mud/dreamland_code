...
CMDRUN( remort )
{
    ...
    if (argument.empty( )) {
        ...
        pch->pecho("Вещи можно сохранить в именном сундуке или собственном доме ({y{hcсправка цены{x, {y{hcсправка строительство{x)");
        pch->pecho("Обязательно прочитай {y{hcсправка перерождение{x");
        pch->pecho("Если хочешь начать новую жизнь, набери {yпереродиться{x пароль.");
        return;
    }
