...
CMDRUN( remort )
{
    ...
    if (IS_SET(pch->comm, COMM_NOTELL|COMM_NOEMOTE) 
        ...
        || pch->in_room->vnum == 10)
    {
        pch->pecho(_("Ты еще не искупи%1$Gло|л|ла провинности ЭТОЙ жизни."), pch);
        return;
    }
    ...
    if (!PCharacterManager::pfRemort(pch)) {
        pch->pecho(_("Не удалось сохранить твой профайл: сообщи Богам!"));
        return;
    }
