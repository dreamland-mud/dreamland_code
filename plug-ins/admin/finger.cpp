...
CMDADM(finger)
{
    ...
    if (isdigit(ipAddress.at(0))) {
        ...
        if (!found) {
            ch->pecho(_("Не найдено никого с таким IP адресом."));
            return;
        }

        ch->pecho(_("Список всех игроков, заходивших с этого или похожего адреса:"));
        page_to_char(str.str().c_str(), ch);
        return;
    } 

    ch->pecho(_("Не найдено никого с таким именем."));
}
