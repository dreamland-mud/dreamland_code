...
static inline bool has_sanctuary_msg( Character *ch, Character *victim )
{
    if (IS_AFFECTED(victim, AFF_SANCTUARY)) {
        if (victim == ch)
            ...
            act("%2$^C1 уже под защитой святилища.", ch, victim, 0,TO_CHAR);
        return true;
    }
    ...
    if (victim->isAffected(gsn_dark_shroud)) {
        if (victim == ch)
            ...
            act("%2$^C1 уже под защитой темных богов.", ch, victim, 0,TO_CHAR);
        return true;
    }
    ...
    if (victim->isAffected(gsn_stardust)) {
        if (victim == ch)
            ...
            act("Звездная пыль уже кружится вокруг %2$C2.", ch, victim, 0,TO_CHAR);
        return true;
    }
...
{ 
    ...
    if (victim->isAffected(sn)) {
        if (victim == ch)
            ...
            oldact("$C1 уже защище$Gно|н|на заклинанием брони.", ch, 0, victim, TO_CHAR);
        return;
    }
    ...
    if (ch->getProfession( )->getFlags( ch ).isSet(PROF_DIVINE)) {
        ...
        if (ch != victim)
            act("Священная броня окружает %2$C4.", ch, victim, 0,TO_CHAR);
    } else {
        act("Волшебная броня окружает тебя.", victim, 0, 0,TO_CHAR);
        if (ch != victim)
            act("Волшебная броня окружает %2$C4.", ch, victim, 0,TO_CHAR);
    }
}
...
{ 
    ...
    if ( ch->isAffected(sn ) )
    {
        if (victim == ch)
          ...
          act("Кожа %2$C2 не может стать еще прочнее.",  ch, victim, 0,TO_CHAR);
        return;
    }
    ...
    affect_to_char( victim, &af );
    act("Кожа %C2 покрывается корой.",             victim,  0,  0, TO_ROOM);
    ...
    if (IS_GOOD(victim)) // Not for good !!!
    {
       if (victim == ch)
          act("Темные боги не будут защищать тебя!!!", ch, 0, 0,TO_CHAR);
                 ...
                 act("Темные боги не будут защищать %2$C4!!!", ch, victim, 0,TO_CHAR);
        return;
    }
...
{ 
  ...
  if ( victim->isAffected(sn ) )
    {
      if (victim == ch)
               ...
               act("Кожа %2$C2 уже тверда, как драконья.",  ch, victim, 0,TO_CHAR);
      return;
    }
  ...
  affect_to_char( victim, &af );
  act("Кожа %C2 становится тверже драконьей.",           victim, 0, 0,TO_ROOM);
...
{ 
    ...
    if ( victim->isAffected(sn ) )
    {
        if (victim == ch)
          ...
          act("Силовое поле уже окружает %2$C4.",ch,victim,0,TO_CHAR);
        return;
    }
    ...
    if ( ch != victim )
        act("Силовое поле окружает %2$C4.",ch,victim,0,TO_CHAR);
    return;
...
{ 
    ...
    if (ch->isAffected(sn )) {
        oldact("Ты уже гото$gво|в|ва противостоять холоду и темным чарам.", ch, 0, 0, TO_CHAR);
        return;
    }
...
{ 
    ...
    for( gch=room->people; gch != 0; gch=gch->next_in_room)
    {
        ...
        if (ch != gch)
            act("{WБелая аура{x окружает %2$C4.", ch, gch, 0,TO_CHAR);
    }
}
...
{ 
    ...
    {
        if (victim == ch)
          oldact("Ты уже защище$gно|н|на от холода.", ch,0, 0,TO_CHAR);
        else
          oldact("$C1 уже защище$Gно|н|на от холода.", ch,0,victim,TO_CHAR);
        return;
    }
    ...
    {
        if (victim == ch)
          oldact("Ты уже защище$gно|н|на от огня.", ch,0, 0,TO_CHAR);
        else
          oldact("$C1 уже защище$Gно|н|на от огня.", ch,0,victim,TO_CHAR);
        return;
    }
    ...
    {
        if (victim == ch)
          oldact("Ты уже защище$gно|н|на ледяным щитом.", ch,0,0,TO_CHAR);
        else
          oldact("$C1 уже защище$Gно|н|на ледяным щитом.", ch,0,victim,TO_CHAR);
        return;
    }
    ...
    if ( ch != victim )
        act("Защищенность %2$C2 от воздействия низких температур повышается.",  ch, victim, 0,TO_CHAR);
    return;
...
{ 
    ...
    {
        if (victim == ch)
          oldact("Ты уже защище$gно|н|на.", ch,0, 0,TO_CHAR);
        else
          oldact("$C1 уже защище$Gно|н|на.", ch,0,victim,TO_CHAR);
        return;
    }
    ...
    if (IS_NEUTRAL(victim))
    {
        if (victim == ch)
          ...
          act("%2$^C1 не может получить защиту от злых существ.", ch,victim,0,TO_CHAR);
        return;
    }
    ...
    if ( ch != victim )
        act("%2$^C1 получает защиту от злых существ.",  ch, victim, 0,TO_CHAR);
    return;
...
{ 
    ...
    if (IS_NEUTRAL(victim))
    {
        if (victim == ch)
          ...
          act("%2$^C1 не может получить защиту от добрых существ.", ch,victim,0,TO_CHAR);
        return;
    }
    ...
    if ( ch != victim )
        act("%2$^C1 получает защиту от добрых существ.",  ch, victim, 0,TO_CHAR);
    return;
...
{ 
    ...
    {
        if (victim == ch)
          oldact("Ты уже защище$gно|н|на огненным щитом.", ch,0,0,TO_CHAR);
        else
          oldact("$C1 уже защище$Gно|н|на огненным щитом.", ch,0,victim,TO_CHAR);
        return;
    }
    ...
    if ( ch != victim )
        act("Защищенность %2$C2 от воздействия высоких температур повышается.",  ch, victim, 0,TO_CHAR);
    return;
...
{ 
  ...
  if (victim->isAffected(sn)) {
      if (victim == ch)
               ...
               act("Охранный щит уже окружает %2$C4.",ch,victim,0,TO_CHAR);
      return;
  }
...
{ 
    ...
    if ( victim->isAffected(sn ) )
    {
        if (victim == ch)
            ...
            act("%2$^C1 уже под воздействием щита.", ch, victim, 0,TO_CHAR);
        return;
    }
...
{ 
    ...
    if ( ch->isAffected(sn ) )
    {
        if (victim == ch)
          ...
          act("Кожа %2$C2 уже тверда как камень.",  ch, victim, 0,TO_CHAR);
        return;
    }
    ...
    affect_to_char( victim, &af );
    act("Кожа %C2 становится тверже камня.",             victim,  0,  0, TO_ROOM);
