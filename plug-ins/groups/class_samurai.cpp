...
SKILL_RUNP( harakiri )
{
    ...
    if(SHADOW(ch)) {
      ...
      oldact("$c1 не может даже сделать себе харакири.\n...пора на пенсию.", ch, 0, 0, TO_ROOM);
      return;
    }
...
SKILL_RUNP( katana )
{
        ...
        if (SHADOW(ch))
        {
                ...
                oldact("$c1 вместе со своей тенью пытаются сделать катану.\n\r...глупое занятие.", ch, 0, 0, TO_ROOM);
                return;
        }
