...
{ 
    ...
    potion->value1(spell);
    extract_obj(obj);
    act("Используя магические силы, ты изготавливаешь %3$O4!",  ch,  0,  potion, TO_CHAR);
      ...
      if(!victim->isAffected(sn) && !saves_spell(level, victim, DAM_MENTAL, ch, DAMF_MAGIC))
        {
          ...
          if (ch != victim) {
            act("Твой ментальный удар повреждает разум %2$C2!", ch,victim,0,TO_CHAR);
            act("Ментальный удар %C2 повреждает твой разум!", ch,victim,0,TO_VICT);
            act("Ментальный удар %1$C2 повреждает разум %2$C2!", ch,victim,0,TO_NOTVICT);
          }
          else {
