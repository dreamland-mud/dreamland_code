...
bool CardStarterBehavior::death( Character *killer ) 
{
    ...
    oldact(_("{cТы уби$gло|л|ла шестерку $n2 из Колоды.{x"), 
            killer, XMLAttributeCards::suitFaces[suit].mlt, 0, TO_CHAR );

    if (card->getLevel( ) < 0) {
        card->setSuit( suit );
        card->setLevel( 0 );
        oldact(_("{cТеперь ТЫ займешь её место.{x"), killer, 0, 0, TO_CHAR );
    }
...
{
    if (!victim->is_npc( ))
        oldact(_("$c1 перетасовывает карты, хитро поглядывая на тебя."), ch, 0, victim, TO_VICT);
}
...
void CardSellerBehavior::speech( Character *victim, const char *speech ) 
{
    ...
    if (victim->getPC( )->getQuestPoints() < 50) {
        oldact(_("$c1 ухмыляется."), ch, 0, 0, TO_ROOM);
        oldact(_("$c1 произносит '{gУ тебя недостаточно дурной славы (qp), чтобы пользоваться моими картами.{x'"), ch, 0, victim, TO_ROOM);
        return;
    }
    
    if (!( pPackIndex = get_obj_index( OBJ_VNUM_CARDPACK ) )) {
        oldact(_("$c1 произносит '{gИзвини, у меня закончились карты.{x'"), ch, 0, 0, TO_ROOM);
        return;
    }
    ...
    oldact(_("$c1 вручает тебе $o4."), ch, pack, victim, TO_VICT);
    oldact(_("$c1 вручает $C3 $o4."), ch, pack, victim, TO_NOTVICT);
}
