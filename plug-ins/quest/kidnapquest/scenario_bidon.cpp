...
void KS::actGiveMark( NPCharacter *king, PCharacter *hero, Object * mark, int time ) const
{
    ...
    if(number_percent() < 10) {
        ...
        msg = fmt(hero, _("$c1 говорит тебе '{GИ учти, что через {Y%1$d{G минут%1$Iу|ы| мое терпение лопнет!{x'"),
                 time );
    } else {
        msg = fmt(hero, _("$c1 говорит тебе '{GПоторопись! Материнское сердце подсказывает мне, "
                      "что если ты не приведешь ее ко мне через {Y%1$d{G минут%1$Iу|ы|, "
