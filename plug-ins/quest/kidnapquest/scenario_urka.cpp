...
{
    act("%2$^C1 поднимается навстречу %1$C3.", kid, king, 0,TO_ROOM);
    act("%2$^C1 внимательно смотрит на %1$C4.", kid, king, 0,TO_ROOM);
...
{
    if (!hero->fighting) {
        act("%^C1 произносит '{gА вас я попрошу задержаться для опознания вашего трупа...{x'.", bandit, hero, 0,TO_ROOM);
        act("%^C1 злобно ухмыляется.", bandit, hero, 0,TO_ROOM);
    }
}
void KS::actBeginKidnap( NPCharacter *bandit, NPCharacter *kid ) const 
{
    act("%1$^C1 защелкивает наручники на запястьях %2$C2 и тащит его прочь.", bandit, kid, 0,TO_ROOM);
}
...
void KS::actKidnapStep( NPCharacter *bandit, NPCharacter *kid ) const 
{
    if(number_percent() < 10) {
        ...
        act("%1$^C1 пинком придает %2$C3 нужное направление.", bandit, kid, 0,TO_ROOM);
    }
}
void KS::actEmptyPath( NPCharacter *bandit, NPCharacter *kid ) const 
{
    if(number_percent() < 10) {
        ...
        if (kid->in_room == bandit->in_room)
            act("%2$^C1 злобно ухмыляется.", bandit, kid, 0,TO_ROOM);
    }
}
...
{
    act("%^C1 говорит тебе '{GХм, пожалуй, тебе можно доверить ответственное дельце. Слухай сюды:{x", king, hero, 0,TO_VICT);
    act("%^C1 говорит тебе '{Gнедавно мусора поганые ни за что, ни про что схапали моего братишку{x", king, hero, 0,TO_VICT);
...
void KS::actGiveMark( NPCharacter *king, PCharacter *hero, Object * mark, int time ) const 
{
    ...
    if(number_percent() < 50) {
        act("%^C1 говорит тебе '{GНо это еще ладно, они хотят его повесить. Понимаешь, невиновного человека повесить!!!{x'", king, hero, 0,TO_VICT);
    } else {
        act("%^C1 говорит тебе '{GНо это еще ладно, представляешь, они говорят, что таких хороших людей{x'", king, hero, 0,TO_VICT);
        act("%^C1 говорит тебе '{Gдолжно быть много и хотят его четвертовать...{x'", king, hero, 0,TO_VICT);
    }

    act("%^C1 говорит тебе '{GНадо бы его выручить! Вот, у меня тут есть его паспорт,{x'", king, hero, 0,TO_VICT);
    act("%^C1 говорит тебе '{Gтак как в наше время без документа никуда, а заодно братишка поймет, что тебе можно доверять...{x'", king, hero, 0,TO_VICT);
...
void KS::actMarkLost( NPCharacter *king, PCharacter *hero, Object * mark ) const 
{
    ...
    act("%^C1 говорит тебе '{GТы думаешь у меня тут художественная мастеркая? Аккуратнее надо!!!{x'", king, hero, 0,TO_VICT);
...
void KS::actAckWaitComplete( NPCharacter *king, PCharacter *hero ) const 
{
    ...
    act("Причем тебе кажется, что сапог движется навстречу.", king, hero, 0,TO_VICT);
    oldact("$c1 так хлопает $C4 по плечу, что $E теряет равновесие и падает мордой $m на сапог.", king, 0, hero, TO_NOTVICT);
    act("Причем сапог явно движется навстречу.", king, hero, 0,TO_NOTVICT);
    act("%^C1 говорит тебе '{GМолодец, вернись к давшему тебе задание!{x'.", king, hero, 0,TO_VICT);
}
...
void KS::actNoHero( NPCharacter *kid, PCharacter *hero ) const 
{
    if (number_percent( ) < 10 && hero && hero->in_room != kid->in_room)
        act("%1$^C1 чешет репу в попытке сообразить, куда же делся этот %2$C1.", kid, hero, 0,TO_ROOM);
}
...
{
    act("%^C1 говорит тебе '{gЭээ нет, мой братуха конечно не очень разборчив в персонале,{x'", kid, victim, 0,TO_VICT);
    act("%^C1 говорит тебе '{gно такого хмыря как ТЫ, даже он не послал бы на выручку.{x'", kid, victim, 0,TO_VICT);
}
void KS::actWrongMark( NPCharacter *kid, Character *victim, Object *obj ) const 
{
    act("%^C1 говорит тебе '{gТы чего мне принес? Совсем мозги в боях повышибали?{x'", kid, victim, 0,TO_VICT);
    act("%^C1 говорит тебе '{gТы на меня посмотри и на эту батву в ксиве... Нормальный докУмент неси.{x'", kid, victim, 0,TO_VICT);
}
...
void KS::actBanditsUnleash( NPCharacter *kid, PCharacter *hero, NPCharacter *bandit ) const 
{
    ...
    act("%1$^C1 поднимает взгляд на %2$C4 и произносит: '{gКак хорошо, вас-то мы и ищем...{x'", bandit, kid, 0,TO_ROOM);
    act("%2$^C1 озабоченно бормочет: '{gЧей же паспорт этот идиот мне принес?{x'", bandit, kid, 0,TO_ROOM);
}
...
void KS::actLegend( NPCharacter *king, PCharacter *hero, KidnapQuest::Pointer quest ) const 
{
    act("%^C1 говорит тебе '{GХм, пожалуй тебе можно доверить ответственное дельце, слушай:'{x", king, hero, 0,TO_VICT);
    act("%^C1 говорит тебе '{GСовсем недавно под внеочередную облаву попал один из моих лучших головор... товарищей то есть.'{x", king, hero, 0,TO_VICT);
    act("%^C1 говорит тебе '{GУ НИХ видите ли указ вышел, чтобы камеры не пустовали.'{x", king, hero, 0,TO_VICT);
...
void KS::actGiveMark( NPCharacter *king, PCharacter *hero, Object * mark, int time ) const 
{
    ...
    if(number_percent() < 50) {
        act("%^C1 говорит тебе '{GВсе бы ничего, но какой-то тип, очень похожий на моего товарища, видать так сильно насолил этой Фемиде, что она аж окаменела от ужаса.{x'", king, hero, 0,TO_VICT);
        act("%^C1 говорит тебе '{GТак и стоит в своем храме, а ее доблестные служители прямо взбесились и без всякого следствия хотят вздернуть моего лучшего голов... товарища.'{x", king, hero, 0,TO_VICT);
        act("%^C1 говорит тебе '{GНадо бы его выручить! Вот, у меня тут есть его паспорт, так как в наше время без документа тяжело, а заодно он поймет, что тебе можно доверять.'{x'", king, hero, 0,TO_VICT);
    } else {
        oldact("$c1 говорит тебе '{GНо это только половина беды, представляешь, ОНИ говорят, что таких хороших людей должно быть много и хотят его четвертовать...{x'", king, 0, hero, TO_VICT);
        oldact("$c1 говорит тебе '{GНадо бы помочь товарищу! Вот, у меня есть его паспорт, так как в наше время без документа никуда, а заодно он поймет, что тебе можно доверять.{x'", king, 0, hero, TO_VICT);
    }
...
void KS::actWrongGiver( NPCharacter *kid, Character *victim, Object *obj ) const
{
    act("%^C1 говорит тебе '{gТы еще кто такой? Давай, давай, топай отсюда,{x'", kid, victim, 0,TO_VICT);
    act("%^C1 говорит тебе '{gНе хватало того, чтобы меня застукали в твоей кампании.{x'", kid, victim, 0,TO_VICT);
}
void KS::actWrongMark( NPCharacter *kid, Character *victim, Object *obj ) const
{
    act("%^C1 говорит тебе '{gЧто ты мне суешь? Все умы по дороге растерял?{x'", kid, victim, 0,TO_VICT);
    act("%^C1 говорит тебе '{gТы на меня посмотри и на то, что в паспорте нарисовано...{x'", kid, victim, 0,TO_VICT);
    act("%^C1 говорит тебе '{gНормальный документ неси.{x'", kid, victim, 0,TO_VICT);
}
void KS::actHeroDetach( NPCharacter *kid, PCharacter *hero ) const 
{
