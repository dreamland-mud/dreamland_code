...
void KS::msgRemoteReunion( NPCharacter *kid, NPCharacter *king, PCharacter *hero ) const
{
    ...
    act("%1$^C1 глядя на %2$C4 произносит: '{gО, вот и обед подоспел{x'.", king, kid, 0,TO_ROOM);
    act("%^C1 хватает дите за руку.", king, 0, 0,TO_ROOM);
    hero->printf( "%s и %s уже встретились.\r\n", king->getNameP( '1' ).c_str( ), kid->getNameP( '1' ).c_str( ) );
    act("Сходи, проведай %2$C4.", hero, king, 0,TO_CHAR);
}
void KS::msgKingDeath( NPCharacter *king, Character *killer, PCharacter *hero ) const
{
    if(hero == killer) {
        ...
        act("{YДля тебя же это означает, что: задание отменяется.{x", killer, hero, 0,TO_VICT);
    }
}
...
void KS::actAttackHero( NPCharacter *bandit, PCharacter *hero ) const 
{
    ...
    if (chance( 10 )) {
        ...
        act("Раскатывающее движение скалкой %C2 {R<*) (*>= ! ПРЕВРАЩАЕТ В КРОВАВОЕ МЕСИВО ! =<*) (*>{x твое лицо", bandit, hero, 0,TO_VICT);
        act("Ты в {RУЖАСНОМ СОСТОЯНИИ.{x", bandit, hero, 0,TO_VICT);
        act("Раскатывающее движение скалкой %1$C2 {R<*) (*>= ! ПРЕВРАЩАЕТ В КРОВАВОЕ МЕСИВО ! =<*) (*>{x лицо %2$C2", bandit, hero, 0,TO_NOTVICT);
        act("%2$^C1 в {RУЖАСНОМ СОСТОЯНИИ.{x", bandit, hero, 0,TO_NOTVICT);
    }
    else {
        ...
        act("%^C1 качает головой.", bandit, hero, 0,TO_ROOM);
    }
}
void KS::actBeginKidnap( NPCharacter *bandit, NPCharacter *kid ) const 
{
    ...
    act("%1$^C1 говорит %2$C3: '{gПойдем, драгоценный мой, а злыдня этого мы проучим.{x", bandit, kid, 0,TO_ROOM);
}
...
{
    if(number_percent() < 10)
        act("%2$^C1, держа %1$C4 за палец, тащит коня-качалку за собой.", bandit, kid, 0,TO_ROOM);
}
void KS::actEmptyPath( NPCharacter *bandit, NPCharacter *kid ) const 
{
    if(number_percent() < 10) {
        ...
        if (kid->in_room == bandit->in_room)
            act("%2$^C1 готов расплакаться.", bandit, kid, 0,TO_ROOM);
    }
}
...
{
    act("%^C1 оценивающе смотрит на тебя.", king, hero, 0,TO_VICT);
    act("%1$^C1 оценивающе смотрит на %2$C4.", king, hero, 0,TO_NOTVICT);
    ...
    act("%^C1 говорит тебе '{GЗамечательный экземплярчик...{x'", king, hero, 0,TO_VICT);
}
void KS::actGiveMark( NPCharacter *king, PCharacter *hero, Object * mark, int time ) const 
{
    ...
    if(number_percent() < 50) {
        ...
        act("%^C1 говорит тебе '{GВот, возьми эту деревяшку, думаю как приманка сработает.{x'", king, hero, 0,TO_VICT);
        act("%1$^C1 дает тебе %3$C4.", king, hero, mark,TO_VICT);
        oldact("$c1 вручает $C3 $o4.", king, mark, hero, TO_NOTVICT);
        act("%^C1 говорит тебе '{GПриведи его сюда.{x'", king, hero, 0,TO_VICT);
...
{
    act("%^C1 говорит тебе '{GЛошадка приглянулась? Ладно, можешь оставить ее себе.{x'", king, hero, 0,TO_VICT);
    act("%1$^C1 дает тебе %3$C4.", king, hero, mark,TO_VICT);
...
void KS::actAckWaitComplete( NPCharacter *king, PCharacter *hero ) const 
{
    ...
    act("%^C1 выжидающе смотрит на тебя.", king, hero, 0,TO_VICT);
    act("%1$^C1 выжидающе смотрит на %2$C4.", king, hero, 0,TO_NOTVICT);
}
...
void KS::actNoHero( NPCharacter *kid, PCharacter *hero ) const 
{
    if (number_percent( ) < 10 && hero && hero->in_room != kid->in_room)
        act("%1$^C1 вот-вот расплачется, потеряв %2$C4.", kid, hero, 0,TO_ROOM);
}
...
{
    act("%^C1 насупив нос отворачивается от тебя", kid, victim, 0,TO_VICT);
    act("%1$^C1 насупив нос отворачивается от %2$C4", kid, victim, 0,TO_NOTVICT);
}
void KS::actWrongMark( NPCharacter *kid, Character *victim, Object * ) const 
{
    act("%^C1 говорит тебе '{GЭто не моя лошадка.{x'", kid, victim, 0,TO_VICT);
}
void KS::actGoodMark( NPCharacter *kid, Character *victim, Object *obj ) const 
{
    interpret_raw(kid, "flip");
    act("%^C1, обняв лошадку, доверчиво смотрит тебе в глаза.", kid, victim, 0,TO_VICT);
    act("%1$^C1, обняв лошадку, доверчиво смотрит в глаза %2$C3.", kid, victim, 0,TO_NOTVICT);
}
