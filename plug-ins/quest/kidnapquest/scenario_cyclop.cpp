...
void KS::actGiveMark( NPCharacter *king, PCharacter *hero, Object * mark, int time ) const 
{
    ...
    if(number_percent() < 50) {
        ...
        msg = fmt(hero, _("$c1 говорит тебе '{GИ поспеши! Чувствую, что через {Y%1$d{G минут%1$Iу|ы| он мне уже не понадобится.{x"),
                 time );
    } else {
        ...
        oldact(_("$c1 дает тебе $o4."), king, mark, hero, TO_VICT);
        oldact(_("$c1 вручает $C3 $o4."), king, mark, hero, TO_NOTVICT);
        msg = fmt(hero, _("$c1 говорит тебе '{GПоспеши! Через {Y%1$d{G минут%1$Iу|ы| он должен быть здесь!{x"),
                 time );
    }
