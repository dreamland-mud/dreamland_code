...
CMDRUNP( affects )
{
    ...
    if (IS_CHARMED(ch)) {
        if (buf.str( ).empty( )) 
            act("%2$^C1 не находится под действием каких-либо аффектов.", ch->master, ch, 0,TO_CHAR);
        else 
            act("%2$^C1 находится под действием следующих аффектов:", ch->master, ch, 0,TO_CHAR);
        buf << "{x";
