...
static void show_char_description( Character *ch, Character *vict )
{
    if(IS_VAMPIRE(vict)){
        ostringstream buf;
        buf << fmt(ch, _("Монстр в своем ужасающем обличии. Нечисть и порождение тьмы.")) << endl
            << fmt(ch, _("Пара ярко-красных глаз и ослепительно острых клыков сверкают на фоне обсидиановой, почти черной, как ночь, кожи.")) << endl
            << fmt(ch, _("Огромное и мускулистое тело ночно%1$Gго|го|й хищни%1$Gка|ка|цы, обрамленное крыльями, какие есть у летучих мышей, замерло в ожидании.\n\r"), vict);
        ch->send_to(buf);
        return;  
    }
