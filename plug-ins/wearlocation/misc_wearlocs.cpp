...
void WieldWearloc::reportWeaponSkill( Character *ch, Object *obj )
{
    ...
    if (skill >= 100)
        msg = _("$o1 становится частью тебя!");
    else if (skill > 85)
        msg = _("Ты чувствуешь себя с $o5 абсолютно уверенно.");
    else if (skill > 70)
        msg = _("Ты хорошо владеешь $o5.");
    else if (skill > 50)
        msg = _("Ты довольно посредственно владеешь $o5.");
    else if (skill > 25) {
        if (sn != gsn_exotic)
            msg = _("Ты чувствуешь себя неуклюже, вооружившись $o5.{/Попрактикуйся еще в умении владеть этим типом оружия.");
        else
            msg = _("Ты все еще не готов{Sfа{Sx как следует владеть $o5.{/Владение экзотическим оружием зависит от твоего опыта и интеллекта.");
    } else {
        if (sn != gsn_exotic)
            msg = _("Ты понятия не имеешь, как орудовать $o5.{/Возможно, стоит попрактиковаться в умении владеть этим типом оружия.");
        else
            msg = _("Ты пока не готов{Sfа{Sx как следует владеть $o5.{/Владение экзотическим оружием зависит от твоего опыта и интеллекта.");
    }
