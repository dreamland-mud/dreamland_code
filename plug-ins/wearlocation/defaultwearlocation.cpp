...
bool DefaultWearlocation::canEquip( Character *ch, Object *obj )
{
    if (obj->isAntiAligned( ch )) {
        act("Твоя натура не позволяет тебе носить %3$O4.", ch, 0, obj,TO_CHAR);
        oldact("$o1 соскальзывает с $c2.", ch, obj, 0, TO_ROOM );
        act("%3$^O1 соскальзывает с тебя.", ch, 0, obj,TO_CHAR);
        return false;
    }
    
    if (!obj->getRealShortDescr( ) && obj->wasAntiAligned( ch )) {
        act("Твоя натура все еще не позволяет тебе носить %3$O4.", ch, 0, obj,TO_CHAR);
        oldact("$o1 соскальзывает с $c2.", ch, obj, 0, TO_ROOM );
        act("%3$^O1 соскальзывает с тебя.", ch, 0, obj,TO_CHAR);
        return false;
    }
    ...
    if (IS_SET( obj->extra_flags, ITEM_MAGIC ) && ch->isAffected( gsn_spellbane )) {
        ...
        act("Магия %3$O2 аннигилирует с твоим спеллбаном!", ch, 0, obj,TO_CHAR);
...
bool DefaultWearlocation::canRemove( Character *ch, Object *obj, int flags )
{
    if (IS_SET(obj->extra_flags, ITEM_NOREMOVE)) {
        if (IS_SET(flags, F_WEAR_VERBOSE)) 
            act("Ты не можешь снять %3$O4, на этой вещи проклятие.", ch, 0, obj,TO_CHAR);
        return false;
    }
